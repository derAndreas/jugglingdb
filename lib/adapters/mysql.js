...
var TransactionQueue = require('../utils').TransactionQueue;
...
exports.initialize = function initializeSchema(schema, callback) {
    ...
    for(i = 0; i < cons; i++) {
        ...
        client._juggledb_name = 'pool-' + i; //todo: debug, remove it later
        pool.push(client);
        
        client.query('USE ' + s.database, function (err) {
            if (err && err.message.match(/^unknown database/i)) {
                var dbName = s.database;
                client.query('CREATE DATABASE ' + dbName, function (error) {
                    if (!error) {
                        client.query('USE ' + s.database, callback);
                    } else {
                        throw error;
                    }
                });
...
function MySQL(pool) {
    ...
    /**
     * Define `client` as a getter to this.getClient()
     * without the ability to get a locked client
     * implemented to have no compability issues
     */
...
MySQL.prototype.getClient = function(locked) {
    if(this.pool.length == 0) {
        throw new Error('No free connections in the pool'); //todo queue this and return a connction when available
    }
    
    var client = this.pool.shift();
...
/**
 * Say the MySQL Adapter to init a new Transaction queue
 * and bind a clint from the connection pool to this transaction.
 * 
 * @return void
 */
MySQL.prototype.startTransaction = function() {
    var client = this.getClient(true);
    return new TransactionQueue(client, this);
}
